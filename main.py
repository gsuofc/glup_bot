...
async def on_message(message):
    ...
    log_to_server(f'message from {message.guild.name}: {message.channel.name}', channel_name='glup-responses')
...
async def duskull(ctx):
    ...
    if shiny_roll == 1:
        ...
        log_to_server(f'Duskull generated for {ctx.author.name} was shiny!', channel_name='glup-logs')
...
async def elevate(ctx):
    ...
    embed = discord.Embed(
        ...
        description=f"You gained {int(exp_roll)} erudition\n{level_progression_message}",
        color=discord.Color.purple()
    )
    await ctx.send(embed=embed)
